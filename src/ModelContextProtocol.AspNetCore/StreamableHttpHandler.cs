    ...
    IServiceProvider applicationServices)
{
    ...
    public async Task HandlePostRequestAsync(HttpContext context)
    {
        ...
        // Check for the Mcp-Lro header to indicate long-running tool call handling
        // The value of the header is a globally unique request ID generated by the client
        if (message is JsonRpcRequest request && request.Method == RequestMethods.ToolsCall
            && context.Request.Headers.TryGetValue("Mcp-Lro", out var lroHeader))
        {
            var lroRequestId = lroHeader!.ToString();
            var toolCallStateService = context.RequestServices.GetService<ToolCallStateService>();
            if (toolCallStateService is null)
            {
                await WriteJsonRpcErrorAsync(context,
                    "Internal Server Error: The server is not configured to handle long-running operations.",
                    StatusCodes.Status500InternalServerError);
                return;
            }
            var toolCallTask = GetToolCallResponseStreamAsync(session, message, context.RequestAborted);

            // For LRO requests, create the initial state and return 202 with Location header
            toolCallStateService!.CreateToolCallState(lroRequestId!, toolCallTask);

            var locationUrl = $"{context.Request.Scheme}://{context.Request.Host}{context.Request.PathBase}/toolCallResult/{lroRequestId}";
    ...
    internal async Task<Stream> GetToolCallResponseStreamAsync(StreamableHttpSession session, JsonRpcMessage message, CancellationToken token)
    {
        var responseStream = new MemoryStream();
        var wroteResponse = await session.Transport.HandlePostRequest(message, responseStream, token);
    ...
    public async Task HandleGetToolCallResultAsync(HttpContext context)
    {
        ...
        var store = context.RequestServices.GetRequiredService<ToolCallStateService>();
        if (!store.ToolCallExists(requestId))
        {
            await WriteJsonRpcErrorAsync(context, "Not Found: Tool call not found", StatusCodes.Status404NotFound);
            return;
        }
